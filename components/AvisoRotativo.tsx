/**
 * A tarja do topo: os recados da loja passando devagar, separados por
...
export function AvisoRotativo({ avisos }: { avisos: string[] }) {
  ...
  return (
    <div className="bg-noite-funda py-1.5">
        ...
        <ul className="marquee marquee--rapida" aria-hidden="true">
          {volta.map((texto, i) => (
            <li key={`${texto}-${i}`} className="flex shrink-0 items-center">
              <span className="px-5 text-[0.75rem] text-cera-fraca">{texto}</span>
